...
pub(super) struct QueryCommand {
    pub statement: String,
    pub arguments: Vec<Value>,
    pub tx: oneshot::Sender<Result<QueryHandle>>,
}
...
impl<'a> QueryTask<'a> {
    pub fn new(stmt: deadpool_sqlite::rusqlite::Statement<'a>, arguments: Vec<Value>) -> Self {
        Self { stmt, arguments }
    }

    pub fn blocking_run(mut self, handle_rx: oneshot::Sender<Result<QueryHandle>>) {
